...
def create_image_indvs_mmd(imgs, num): 
    ...
    for i in range(num-1):
        ...
        for img in imgs:
            # print(img.shape)
            img=np.transpose(img, (1, 2, 0))
            tmp_indivs.append(Mutators.image_random_mutate_ga(img)[0])
        indivs.append(np.array(tmp_indivs))
        # print(indivs[1].shape)
    return indivs
...
def predict(input_data, model):
    ...
    try:
        ...
        model.eval()
        with torch.no_grad():
            # 如果输入数据的形状是 (224, 224, 3)，则进行维度转置
            if input_data.shape[1:] == (224, 224, 3):
                input_data=np.transpose(input_data,(0,3,1,2))
            model(torch.Tensor(input_data))
        remove_hooks()
        outputs = [hook.get_outputs() for hook in hooks]
        handles.clear()
        hooks.clear()
    finally:
        pass
...
def main():
    ...
    parser.add_argument('-target', help='target class for optimization', type=int, default=-1)
    ...
    # Load a model
    preprocess = empty_processing # 加载数据集对应的预处理函数
    ...
    if orig_imgs.shape[0] < pop_num:
        while orig_imgs.shape[0] < pop_num:
            orig_imgs = np.concatenate((orig_imgs, orig_imgs), axis=0)
        orig_imgs = orig_imgs[:pop_num]
    print(orig_imgs.shape)
    # pop_num=orig_imgs.shape[0]
    model.eval()
    ground_truth = np.argmax(model(preprocess(orig_imgs)).detach().numpy()[0]) # 记录gtruth，由于我的seed中的图像都是来自相同类别的，所以只保留了一个
    ...
    if args.target != -1:
        ...
        rest=args.i.split("/")[-1]
        len_rest=len(rest)
        nes_path=nes_path[:-(len_rest)]
        nes_pth_list = os.listdir(nes_path)
        nes_pth_list.sort()
    
    
    nes_pth_list.sort(key=lambda x:int(x.split("_")[1]))
    if nes_pth_list is not None:
        nes = []
        for p in nes_pth_list:
            if not target==int(p.split("_")[1]):
                nes.append([])
                continue
    ...
    print(1)
    file_path="/home/ubuntu/hbl/LargeD/ProCMG/IN100/DistXplore/DistXplore/dist-guided/cnt.pickle"
    cnt=0
    ...
    print(cnt)
    shutil.rmtree(os.path.join(ga_output,"crashes"))
    shutil.rmtree(os.path.join(ga_output,"imgs"))
