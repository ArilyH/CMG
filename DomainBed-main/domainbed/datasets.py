...
class MultipleEnvironmentImageFolderGen(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, gen_envs, hparams):
        super().__init__()
        environments = [f.name for f in os.scandir(root) if f.is_dir()]
        environments = sorted(environments)
        ...
        for i, environment in enumerate(environments):
            ...
            path = os.path.join(root, environment)
            if i in gen_envs:
                add_path.append(os.path.join(path,'add'))
            ...
            self.datasets.append(env_dataset)
        # self.datasets.append([])
        add_path.append("/home/ubuntu/hbl/LargeD/ProCMG/OfficeHome/ShortClus/ComClus/train")
        for i in add_path:
            env_transform = augment_transform
            self.datasets.append(ImageFolder(i, transform=env_transform))
        self.input_shape = (3, 224, 224,)
        self.num_classes = len(self.datasets[-1].classes)
class MultipleEnvironmentImageFolderBS(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, gen_envs, BS_dir, hparams):
        super().__init__()
        environments = [f.name for f in os.scandir(root) if f.is_dir()]
        environments = sorted(environments)
        print(environments)
        ...
        for i, environment in enumerate(environments):
            ...
            self.datasets.append(env_dataset)
        # self.datasets.append([])
        add_path=[BS_dir,"/home/ubuntu/hbl/LargeD/ProCMG/OfficeHome/ShortClus/ComClus/train"]
        print(add_path)
        # exit(0)
        for i in add_path:
            env_transform = augment_transform
...
class OfficeHomeGen(MultipleEnvironmentImageFolderGen):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "R"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "office_home/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'],test_envs, hparams)
class OfficeHomeBS(MultipleEnvironmentImageFolderBS):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "R"]
    def __init__(self, root, test_envs, BS_dir,hparams):
        self.dir = os.path.join(root, "office_home/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'],test_envs,BS_dir, hparams)

class Logo2K(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "Cl", "Co", "E", "F", "I", "L", "M", "N", "T"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "office_home/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
