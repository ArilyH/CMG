...
class ERM(Algorithm):
    ...
    def update_few(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches]+[x for x, y in unlabeled])
        all_y = torch.cat([y for x, y in minibatches]+[y for x, y in unlabeled])
        loss = F.cross_entropy(self.predict(all_x), all_y)
