...
class _SplitDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, key):
        # print(self.underlying_dataset[self.keys[key]])
        return self.underlying_dataset[self.keys[key]]
    def __len__(self):
        return len(self.keys)
    
# class _SplitDatasetGen(torch.utils.data.Dataset):
#     """Used by split_dataset"""
#     def __init__(self, underlying_dataset, keys):
#         super(_SplitDataset, self).__init__()
#         self.underlying_dataset = underlying_dataset
...
#     def __len__(self):
#         return len(self.keys)
#     def __getitemx__(self, key, gene):
#         pass
...
def combine_split(dataset_1, dataset_2, seed=0):
    eles_1=dataset_1.underlying_dataset
    eles_1=Subset(eles_1,dataset_1.keys)
    eles_2=dataset_2.underlying_dataset
    eles_2=Subset(eles_2,dataset_2.keys)
    
    print(len(eles_1),len(eles_2))
    
    dataset=ConcatDataset([eles_1,eles_2])
    keys=list(range(len(dataset)))
    np.random.RandomState(seed).shuffle(keys)
    return _SplitDataset(dataset, keys)

def splitForGen(dataset):
    keys = list(range(len(dataset)))
    return _SplitDataset(dataset, keys)
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            for i in range(len(y)):
                if argp[i]==y[i]:
                    accu[y[i].item()]=accu.get(y[i].item(),0)+1
            if p.size(1) == 1:
            ...
            # print(correct-corbk," ",y)
    network.train()

    return correct / total, accu
